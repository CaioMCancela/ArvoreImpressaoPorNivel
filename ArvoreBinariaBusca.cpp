...
void ArvoreBinariaBusca_Imprimir_Por_Nivel(int nivel) {
    int NivelAtual = 0;
    queue<TipoCelula*> FilaNivel;
    if (Raiz == NULL) {
        cout << "A arvore nao possui dados." << endl;
        return;
    }
    FilaNivel.push(Raiz);
    while (!FilaNivel.empty()) {
        int tamanho = FilaNivel.size();
        if (NivelAtual == nivel) {

            for (int i = 0; i < tamanho; ++i) {
                TipoCelula* atual = FilaNivel.front();
                cout << atual->Item << " ";
                FilaNivel.pop();
            }
            cout << endl;
        ...
        for (int i = 0; i < tamanho; ++i) {
            ...
            FilaNivel.pop();
            if (atual->Esq != NULL) {
                FilaNivel.push(atual->Esq);
            }
            if (atual->Dir != NULL) {
                FilaNivel.push(atual->Dir);
            }
        }
        NivelAtual++;
    }
    cout << "Nivel nao existe na arvore, verifique e tente novamente..."<< endl << endl;
}
